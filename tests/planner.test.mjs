...
const destination = { lat: 41.40, lon: 2.18 };
const candidates = [
...
test('never silently drops mandatory stops when impossible', () => {
  const result = plan({ candidates, origin, destination, start: 540, deadline: 580, pinned: ['museum'] });
  assert.deepEqual(result.route.map(p => p.id), ['museum']);
  assert.equal(result.feasible, false);
});
test('extra time preserves current mandatory stop and recalculates the rest', () => {
  const base = { candidates, origin, destination, start: 540, deadline: 760, first: 'breakfast', pinned: ['breakfast'] };
...
test('skipping removes only that stop; finished stops stay outside replanning', () => {
  const result = plan({ candidates: candidates.slice(2), origin: candidates[0], destination, start: 590, deadline: 1000 });
...
test('transit uses the same next destination as walking, not trip endpoint', () => {
  ...
  assert.equal(transit.searchParams.get('destination'), `${candidates[0].lat},${candidates[0].lon}`);
...
test('slower walking changes the estimate; mandatory stops report lost margin', () => {
  const input = { candidates, origin, destination, start: 540, deadline: 1000, pinned: candidates.map(p => p.id) };
  assert.ok(plan({ ...input, tempo: 'Lugnt' }).transport > plan({ ...input, tempo: 'Högt' }).transport);
});
test('visit countdown does not shift planned departure as time passes', () => {
  const stop = candidates[0], until = 640;
  const before = plan({ candidates: [{ ...stop, minutes: until - 600 }], origin: stop, destination, start: 600, deadline: 800, pinned: [stop.id] });
  const after = plan({ candidates: [{ ...stop, minutes: until - 620 }], origin: stop, destination, start: 620, deadline: 800, pinned: [stop.id] });
...
test('actual walking legs including return leg control feasibility', async()=>{
 ...
 const stop=candidates[0];const legs={
  [legKey(origin,stop,'Normalt')]:{minutes:35,km:2,shape:[]},
  [legKey(stop,destination,'Normalt')]:{minutes:80,km:5,shape:[]}
 };
 const result=plan({candidates:[stop],origin,destination,start:600,deadline:780,pinned:[stop.id],walkingLegs:legs});
 assert.equal(result.route[0].arrival,635);assert.equal(result.arrival,770);assert.equal(result.transport,115);assert.equal(result.feasible,false);
});
test('New York clock handles local date and overnight without using device timezone',async()=>{
 ...
 assert.deepEqual(localClock('America/New_York',new Date('2026-09-05T02:30:00Z')),{date:'2026-09-04',minutes:1350});
 assert.equal(minuteOnTrip('2026-09-04','America/New_York',new Date('2026-09-05T05:30:00Z')),1530);
 assert.equal(minuteOnTrip('2026-09-05','America/New_York',new Date('2026-09-05T05:30:00Z')),90);
 assert.deepEqual(localClock('America/New_York',new Date('2026-12-05T15:00:00Z')),{date:'2026-12-05',minutes:600});
});
