...
const places = [0, 1, 2, 3, 4].map((n) => ({
  id: n,
  category: n < 3 ? 'Restauranger' : n === 3 ? 'Klädbutiker' : 'Sevärdheter',
  lat: 40.754 + n * 0.001,
  lon: -73.9832,
  minutes: n < 3 ? 70 : 45,
...
test('closed shop is not scheduled; late opening waits and entire visit must fit', () => {
  ...
  assert.equal(closed.route.length, 0);
  assert.equal(closed.feasible, false);
  assert.equal(closed.conflicts.length, 1);
});
