...
test('every displayed detailed choice maps to a real search', () => {
  ...
  for (const category of categoryDefinitions)
    for (const choice of category.options)
      assert.ok(optionQueries(category.key, [choice]).length, `${category.key}: ${choice}`);
  for (const choice of categoryDefinitions[0].options.filter((o) => o !== 'Alla'))
    assert.ok(foodTerms[choice]?.length, choice);
});
test('clothing subtype does not confuse menswear with womenswear', () => {
  assert.equal(
    matchesCategory({ shop: 'clothes', clothes: 'women' }, 'Klädbutiker', ['Herr']),
    false,
  );
  assert.equal(
    matchesCategory({ shop: 'clothes', clothes: 'men;women' }, 'Klädbutiker', ['Herr']),
    true,
  );
});
