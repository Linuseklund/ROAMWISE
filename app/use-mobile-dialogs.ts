...
export function useMobileDialogs(open: boolean, onClose: () => void) {
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      document.body.classList.toggle('keyboard-open', !!editing && window.innerHeight - (viewport?.height ?? window.innerHeight) > 120);
    };
    update();
  ...
  useEffect(() => {
    ...
    const background = Array.from(document.querySelector('main')?.children ?? []).filter(el => !el.contains(dialog)) as HTMLElement[];
    ...
    const focusable = () => Array.from(dialog.querySelectorAll<HTMLElement>('button:not(:disabled),a[href],input:not(:disabled),select:not(:disabled),summary,iframe,[tabindex="0"]')).filter(el => el.getClientRects().length);
    ...
    return () => {
      ...
      background.forEach((el, index) => { if (!inertBefore[index]) el.removeAttribute('inert'); });
