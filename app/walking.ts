...
export const legKey = (a: Point, b: Point, tempo: string) =>
  `${a.lat.toFixed(5)},${a.lon.toFixed(5)}>${b.lat.toFixed(5)},${b.lon.toFixed(5)}:${tempo}`;
export function decodeShape(value: string): [number, number][] {
  ...
  const read = () => {
    let result = 0,
      ...
      byte;
    do {
      if (index >= value.length || shift > 30) throw new Error('Invalid route shape');
