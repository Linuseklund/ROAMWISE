...
export default function DayControls({city,meals,onMeals,preferences,onPreferences,activities,onActivities,selected,onChoose,transport,onTransport,endAddress,endTime}:{city:string;meals:MealTimes;onMeals:(v:MealTimes)=>void;preferences:MealPreferences;onPreferences:(v:MealPreferences)=>void;activities:ActivityPreferences;onActivities:(v:ActivityPreferences)=>void;selected:string[];onChoose:(meal:string)=>void;transport:TransportMode;onTransport:(v:TransportMode)=>void;endAddress:string;endTime:string}){
 ...
 <label>Färdsätt<select value={transport} onChange={e=>onTransport(e.target.value as TransportMode)}><option value="walking">Promenad</option><option value="mixed">Promenad + lokaltrafik</option></select></label>
 {transport==='mixed'&&<p>Vid längre sträckor räknar vi med en uppskattad tid för lokaltrafik, inklusive gång och väntan. Kontrollera avgången i kartlänken; tidtabell och störningar är inte hämtade.</p>}
 ...
 {pref.placeId&&<button type="button" onClick={()=>update({placeId:undefined,placeName:undefined,atDestination:false})}>Låt appen föreslå restaurang</button>}
 {meal==='Middag'&&<label className="day-check"><input type="checkbox" checked={!!pref.atDestination} onChange={e=>{update({atDestination:e.target.checked,strict:e.target.checked||pref.strict});if(e.target.checked)onMeals({...meals,[meal]:endTime});}}/>Avsluta med middag på slutadressen</label>}
